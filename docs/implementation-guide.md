...
1. ✅ `rusqlite`(bundled) で run/evaluation を永続化（`--db`、`src/store.rs`）。
2. ✅ 過去 run との差分比較＝リグレッション検出（`store::find_regressions`）+ `safecode history`。
3. ✅ `wasmtime` + WASI で候補を `wasm32-wasip1` にビルドし、fuel/メモリ制限付き埋め込み
   ランタイムでエントリ関数を実行（capability ベース隔離、`--wasm-entry`、`src/wasm.rs`）。
   resource_usage 軸に反映。
...
```bash
# 永続化 + リグレッション検出
safecode evaluate a.rs --db hist.db   # 2 回目に改悪 → リグレッション警告
safecode history --db hist.db          # run 履歴一覧

# Wasm サンドボックス（候補に pub fn run() がある場合）
safecode evaluate a.rs --wasm-entry run                    # Wasm ✅ → resource_usage 加点
safecode evaluate loop.rs --wasm-entry run --wasm-fuel 100000  # fuel 枯渇 → ⏱
```
...
- ✅ Rust mutation testing（`cargo-mutants`、`--mutation`、ADR-009 / Phase 8）
- ✅ Python mutation testing（`mutmut`、Phase 8b）
- Go / JavaScript 対応（同じ `StageResults` パターンで追加可能）
- Go mutation testing（`gremlins`、Phase 8c）
