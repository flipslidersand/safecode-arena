...
//! compile → test → 採点 までを実行する（Phase 1）。
...
pub fn evaluate_candidate(candidate: &Candidate, timeout: Duration) -> Result<Evaluation> {
    ...
    // test（compile 成功時のみ。候補内 #[cfg(test)] を実行）
    ...
    let score = scoring::score(&compile, &test);
    Ok(Evaluation {
