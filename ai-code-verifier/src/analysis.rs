...
///
/// `eslint --format compact` は `file.js: line N, col M, severity - message  (rule)` 形式。
/// `eslint` (default format) は problem 行を含む。`N problem` でカウント。
pub fn count_eslint_findings(output: &str) -> usize {
    // compact: "N problems" or "N problem"
    for line in output.lines() {
        let t = line.trim();
    ...
    // fallback: count diagnostic lines (contain ".js:" and "error" or "warning")
    output
        .lines()
        .filter(|l| {
            let t = l.trim();
...
mod tests {
    ...
    fn count_staticcheck_finds_diagnostics() {
        let out = "candidate.go:5:2: S1000 use plain channel send or receive instead of single-case select (S1000)\n";
