...
fn run_go_stages(
    ...
    let (lint, lint_warnings) = if staticcheck_available() {
        ...
        let warnings = count_staticcheck_findings(&out);
        (outcome, warnings)
    } else {
        let mut l = Command::new("sh");
        l.arg("-c")
        ...
        let warnings = count_go_vet_findings(&out);
        (outcome, warnings)
    };
...
fn run_js_stages(
    ...
    // lint: ESLint があれば実行（--no-eslintrc で設定なし、recommended ルールのみ）
    let (lint, lint_warnings) = if eslint_available() {
        let mut l = Command::new("sh");
        // --no-eslintrc は ESLint v8 以下、v9+ は --no-config-lookup
        // 共通ルール: no-undef, no-unused-vars を最小指定
        l.arg("-c")
        ...
        let warnings = count_eslint_findings(&out);
        (outcome, warnings)
    } else {
